...
// extractVideoFrameFromElement 从已就绪的 <video> 元素按当前播放时间抽取一帧（JPEG）。
// 同源素材路径（/assets 代理）不污染 canvas，可直接用于"重选封面帧"。
export const extractVideoFrameFromElement = (video: HTMLVideoElement): Promise<Blob> => {
  return new Promise((resolve, reject) => {
    try {
      const canvas = document.createElement('canvas')
      const targetWidth = 640

...
const downloadService = {
  getDownloadUrl(gameId: string): string {
    return buildApiUrl(`/games/${gameId}`)
  },

  getFileDownloadUrl(gameId: string, fileId: string): string {
    return buildApiUrl(`/games/${gameId}/files/${fileId}/download`)
  },

  getLaunchScriptUrl(gameId: string, fileId: string): string {
    return buildApiUrl(`/games/${gameId}/files/${fileId}/launch-script`)
  },

  downloadGameVersion(gameId: string, fileId: string): void {
    // File downloads are browser navigations, so they cannot rely on axios baseURL.
  ...
  downloadLaunchScript(gameId: string, fileId: string): void {
    // Launch script downloads follow the same browser-driven path as file downloads.
    const link = document.createElement('a')

...
6. 脚本会先显示菜单：
   - `1` 挂载 SMB 并继续挂载差分 VHD
   - `2` 删除这个脚本刚刚使用的 SMB 凭据，并断开当前共享连接
7. 选择 `1` 后，脚本会自动：
   - 写入 SMB 凭据
   - 连接共享目录
   - 检查本地差分盘是否存在
...
- [start-dev.sh](/home/Hao/Game/Game/start-dev.sh)
  本地开发一键启动
- [build-release.sh](/home/Hao/Game/Game/build-release.sh)
  生成可部署发布包

## 补充说明

- 根目录 README 现在优先服务“如何使用这个项目”
- 更偏后端实现细节的说明可以看 [backend/README.md](/home/Hao/Game/Game/backend/README.md)

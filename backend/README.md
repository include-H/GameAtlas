...
4. PS 主脚本：复用已连接的 SMB 共享（`net use` 检测命中则跳过凭据注入）→ **差分盘已挂载则直接复用**（`Get-VHD` 检测 `Attached`，attach 失败时用 diskpart `detail vdisk` 复核"已附加"降级复用）→ 差分盘不存在则 `create vdisk ... parent=...`，已存在则直接 `attach` → `diskpart` 挂载 → 盘符定位（新挂载用挂载前后 `Get-Partition` diff 取新增盘，复用场景用 `Get-VHD` 的 `DiskNumber` 精确定位）→ 自动扫描游戏 exe（限深 3、排除安装器，多候选列出选择）→ 可配置存档目录（`explorer` 打开，模板支持 `%USERPROFILE%` / `%GAME_DRIVE%` 等变量）；
5. 差分盘路径为 `<VHD_DIFF_ROOT>\<基础盘文件名>`，盘符由 `VHD_DIFF_ROOT` 规范化（非盘符输入回退 `C:`）；
6. 菜单：挂载并游玩（自动启动游戏，结束后自动卸载 VHD、断开共享并删除凭据）、仅挂载、打开存档目录（配置了 `games.save_path_template` 时出现；模板引用 `%GAME_DRIVE%` 才挂载 VHD，本地路径模板直接打开文件夹不碰 SMB/VHD）、删除 SMB 凭据 / 断开共享。
...
- SteamGridDB：按 SGDB 游戏 id 获取 grids / heroes / logos（SGDB 搜索选中后），需要 API Key；

...
func New(cfg config.Config, db *sqlx.DB) *gin.Engine {
	...
	gameAggregateService := services.NewGameAggregateService(cfg, gamesRepo, metadataRepo, gameCatalogService, assetCleanupTasksRepo)
	...
	assetsService := services.NewAssetsService(cfg, gamesRepo, assetsRepo, assetCleanupTasksRepo)

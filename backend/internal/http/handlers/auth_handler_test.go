...
func TestAuthHandlerLoginSetsCookieOnSuccess(t *testing.T) {
	...
	if cookies[0].Secure {
		t.Fatalf("cookie Secure = true, want false for plain HTTP requests")
	}
}

func TestAuthHandlerLoginSetsSecureCookieForHTTPSRequest(t *testing.T) {
	t.Setenv("GIN_MODE", gin.TestMode)
	...
	if len(cookies) == 0 || !cookies[0].Secure {
		t.Fatalf("expected Secure auth cookie for HTTPS request, got %#v", cookies)
	}
}
...
func TestAuthHandlerLogoutClearsCookie(t *testing.T) {
	...
	session, err := service.Login("secret", service.SourceKey("127.0.0.1", ""))
	if err != nil {
		t.Fatalf("Login returned error: %v", err)
	}
	context.Request = httptest.NewRequest(http.MethodPost, "/api/auth/logout", nil)
	context.Request.AddCookie(&http.Cookie{Name: services.AuthCookieName, Value: session})
...
func TestAuthHandlerLogoutClearsSecureCookieForForwardedHTTPSRequest(t *testing.T) {
	t.Setenv("GIN_MODE", gin.TestMode)
	...
	cookies := recorder.Result().Cookies()
	if len(cookies) == 0 || !cookies[0].Secure {
		t.Fatalf("expected cleared auth cookie to keep Secure on forwarded HTTPS request, got %#v", cookies)
	}
	if service.IsAdmin(session) {
		t.Fatalf("expected logout to revoke session")
	}

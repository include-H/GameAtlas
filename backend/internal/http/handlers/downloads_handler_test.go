...
func TestDownloadsHandlerDownloadFormatsChineseContentDisposition(t *testing.T) {
	t.Setenv("GIN_MODE", gin.TestMode)
	...
	root := t.TempDir()
	romPath := filepath.Join(root, "downloads", `使命召唤：现代战争"重制版".rom`)
	if err := os.MkdirAll(filepath.Dir(romPath), 0o755); err != nil {
		t.Fatalf("MkdirAll returned error: %v", err)
	}
	content := []byte("rom-data")
	...
	gameID := insertGamesHandlerTestGame(t, db, "download-chinese", "Download Chinese", domain.GameVisibilityPublic, "")
	fileID := insertDownloadsHandlerGameFile(t, db, gameID, romPath, 0)
	service := services.NewGameFilesService(
		config.Config{PrimaryROMRoot: root},
		repositories.NewGamesRepository(db),
	...
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Request = httptest.NewRequest(http.MethodGet, "/api/downloads/download-chinese/files/"+strconv.FormatInt(fileID, 10), nil)
	context.Params = gin.Params{
		{Key: "publicId", Value: "download-chinese"},
		{Key: "fileId", Value: strconv.FormatInt(fileID, 10)},
	}
	...
	if got := recorder.Header().Get("Content-Disposition"); !strings.Contains(got, "attachment") || !strings.Contains(got, "filename*=") {
		t.Fatalf("Content-Disposition = %q, want RFC 5987 filename parameter", got)
	}
}
...
func TestDownloadsHandlerLaunchScriptFormatsChineseContentDisposition(t *testing.T) {
	t.Setenv("GIN_MODE", gin.TestMode)
	...
	root := t.TempDir()
	romPath := filepath.Join(root, "launch", "game.vhdx")
	if err := os.MkdirAll(filepath.Dir(romPath), 0o755); err != nil {
		t.Fatalf("MkdirAll returned error: %v", err)
	}
	...
	gameID := insertGamesHandlerTestGame(t, db, "launch-chinese", `使命召唤：现代战争"重制版"`, domain.GameVisibilityPublic, "")
	fileID := insertDownloadsHandlerGameFile(t, db, gameID, romPath, 0)
	service := services.NewGameFilesService(
		config.Config{
			PrimaryROMRoot: root,
	...
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Request = httptest.NewRequest(http.MethodGet, "/api/downloads/launch-chinese/files/"+strconv.FormatInt(fileID, 10)+"/launch", nil)
	context.Params = gin.Params{
		{Key: "publicId", Value: "launch-chinese"},
		{Key: "fileId", Value: strconv.FormatInt(fileID, 10)},
	}
...
func TestDownloadsHandlerLaunchScriptSetsSafeAttachmentFilename(t *testing.T) {
	t.Setenv("GIN_MODE", gin.TestMode)
	...
	gameID := insertGamesHandlerTestGame(t, db, "launch-safe-name", `使命召唤：现代战争`, domain.GameVisibilityPublic, "")
	fileID := insertDownloadsHandlerGameFile(t, db, gameID, romPath, 0)
	service := services.NewGameFilesService(
		config.Config{
			PrimaryROMRoot:  root,
	...
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Request = httptest.NewRequest(http.MethodGet, "/api/games/launch-safe-name/files/"+strconv.FormatInt(fileID, 10)+"/launch-script", nil)
	context.Params = gin.Params{
		{Key: "publicId", Value: "launch-safe-name"},
		{Key: "fileId", Value: strconv.FormatInt(fileID, 10)},
	}
	...
	if got := recorder.Header().Get("Content-Disposition"); got != `attachment; filename*=utf-8''%E4%BD%BF%E5%91%BD%E5%8F%AC%E5%94%A4%EF%BC%9A%E7%8E%B0%E4%BB%A3%E6%88%98%E4%BA%89.bat` {

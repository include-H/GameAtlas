...
func TestRunMigrationsOnFreshDatabaseAppliesAllEmbeddedFiles(t *testing.T) {
	...
	assertTableExists(t, db, "start_screen_columns")
	assertColumnExists(t, db, "start_screen_tiles", "image_path")
	assertColumnExists(t, db, "start_screen_tiles", "focus_x")
	assertColumnExists(t, db, "start_screen_tiles", "focus_y")
	assertColumnExists(t, db, "start_screen_tiles", "flip_images")
	assertColumnExists(t, db, "start_screen_tiles", "column_index")
	assertColumnExists(t, db, "start_screen_tiles", "grid_row")
...
func assertColumnMissing(t *testing.T, db *sqlx.DB, table string, column string) {
	...
	for _, row := range rows {
		if row.Name == column {
			t.Fatalf("column %q unexpectedly exists on table %q", column, table)
		}
	}
}

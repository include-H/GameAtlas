...
func getPendingIssueEvaluation(game domain.Game, screenshotCount int64, logoCount int64, videoCount int64, fileCount int64, developerCount int64, publisherCount int64, reviewIssueOverridesRepo *repositories.ReviewIssueOverrideRepository) (*domain.PendingIssueEvaluation, error) {
	if reviewIssueOverridesRepo == nil {
		evaluation := domain.EvaluatePendingIssues(game, screenshotCount, logoCount, videoCount, fileCount, developerCount, publisherCount, nil)
	...
	evaluation := domain.EvaluatePendingIssues(game, screenshotCount, logoCount, videoCount, fileCount, developerCount, publisherCount, ignoredReasons)
	return &evaluation, nil
}
